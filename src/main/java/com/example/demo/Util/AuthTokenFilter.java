 ...
 * 
 * private UserDetailsService customUserDetailsService; private String
 ...
 * ServletException { try { HttpServletRequest httpServletRequest =
 * (HttpServletRequest) servletRequest; String authToken =
 * httpServletRequest.getHeader(authTokenHeaderName);
 * 
 * if (StringUtils.hasText(authToken)) { String username =
 ...
 * 
 * filterChain.doFilter(servletRequest, servletResponse); } catch (Exception ex)
